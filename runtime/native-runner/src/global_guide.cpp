...
// Fixed iteration budget for SUO — ensures deterministic guidance paths
// regardless of wall-clock time.
static constexpr int MAX_SUO_LOOPS = 10;
...
void GlobalGuide::run_suo()
{
  ...
  auto loop = 0;
  while (loop < MAX_SUO_LOOPS && (loop < 2 || CT.collision_cnt < collision_cnt_last)) {
    ++loop;
